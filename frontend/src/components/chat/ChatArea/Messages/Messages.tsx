...
const ReferenceItem: FC<ReferenceItemProps> = ({ reference }) => (
  <div className="relative flex h-[63px] w-[190px] cursor-default flex-col justify-between overflow-hidden rounded-md bg-gray-50 p-3 transition-colors hover:bg-gray-100 border border-gray-200">
    <p className="text-sm font-medium text-gray-900">{reference.name}</p>
    <p className="truncate text-xs text-gray-500">{reference.content}</p>
  </div>
)
    ...
    <div className="flex flex-col gap-y-9">
      ...
      {message.tool_calls && message.tool_calls.length > 0 && (
        <div className="flex items-start gap-3">
          <Tooltip
            delayDuration={0}
            content={<p className="text-gray-700">Tool Calls</p>}
            side="top"
          >
            <Icon
              type="hammer"
...
const ToolComponent = memo(({ tools }: ToolCallProps) => (
  <div className="cursor-default rounded-full bg-gray-100 px-2 py-1.5 text-xs border border-gray-200">
